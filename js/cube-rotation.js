...
export class CubeRotation {
    ...
    // Sequence-ல் இருந்து NEXT step rotate செய்ய
    next() {
        if (this.animating || this.currentView >= this.sequence.length) return false;
    ...
    // Previous step-க்கு exact-ஆக REVERSE செய்ய
    previous() {
        if (this.animating || this.history.length === 0) return false;
