...
// Cube solver initialization check
...
async function startSplash() {
    ...
    setTimeout(() => {
        if (typeof camera !== 'undefined' && typeof renderer !== 'undefined' && typeof viewer !== 'undefined') {
...
function setActiveColor(color) {
    ...
    const activeEl = document.querySelector(`[data-color="${color}"]`);
...
function validateCube() {
    ...
    for (const face of Object.keys(counts)) {
        if (counts[face] !== 9) {
            showToast(`${face} count is ${counts.face}, must be 9.`);
            return false;
        }
    }
...
/* ==========================================
   Solve Button Event (UPGRADED for Animation Queue)
========================================== */
...
const camera = new THREE.PerspectiveCamera(
    35,
    viewer ? viewer.clientWidth / viewer.clientHeight : 1,
    0.1,
    1000
);
...
/* ==========================================
   Raycaster & Coloring Interaction
========================================== */
...
/* ==========================================
   Solver Integration & Animation Queue
========================================== */

async function solveCube(cubeStateData) {
    ...
    try {
        ...
        showToast(`Solved in moves! Processing animation...`);

        // [UPGRADE]: சால்வ் ஆன மூவ்ஸ்களை ஸ்பேஸ் வைத்து பிரித்து அனிமேஷன் யூக்யூவில் (Queue) இயக்குவது
...
// சால்வ் மூவ்ஸ்களை வரிசையாக அனிமேட் செய்யக்கூடிய புதிய பங்க்ஷன்
